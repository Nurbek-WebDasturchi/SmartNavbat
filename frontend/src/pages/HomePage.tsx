      ...
      <section className="relative overflow-hidden bg-gradient-to-br from-slate-900 via-slate-800 to-brand-900 min-h-[80vh] flex items-center">
          ...
          <div>
            <motion.div
              ...
              <p className="text-slate-300 text-lg leading-relaxed mb-8 max-w-md">
                Toshkentdagi davlat klinikalarida navbat kutishga chek qo'ying.
                Uydan navbat oling va faqat kerak vaqtda boring.
              </p>
              ...
              <div className="flex flex-wrap gap-3">
                ...
                <a
                  href="#map"
                  className="btn-ghost flex items-center gap-2 text-base bg-white/10 border-white/20 text-white hover:bg-white/20">
              ...
              {loadingStats ? (
                ...
                <div className="space-y-3">
                  <StatRow
                    ...
                    label="Faol klinikalar"
                    value={<AnimatedNumber value={displayStats.openClinics} />}
                    sublabel={`Jami ${displayStats.totalClinics} ta`}
                  />
                  <StatRow
                    icon={<Users className="w-5 h-5 text-brand-500" />}
                    label="Hozirgi navbat"
                    value={<AnimatedNumber value={displayStats.currentQueue} />}
                    sublabel="Kutayotgan"
                  />
                  <StatRow
                    icon={<CheckCircle className="w-5 h-5 text-brand-500" />}
                    label="Bugun qabul qilindi"
                    value={<AnimatedNumber value={displayStats.todayServed} />}
                    sublabel="bemor"
                  />
                </div>
              ) : null}
        ...
        <div className="absolute bottom-0 left-0 right-0">
          <svg
            ...
            <path
              d="M0 60L1440 60L1440 20C1200 60 900 0 720 20C540 40 240 0 0 20V60Z"
              fill="#f8fafc"
            />
          </svg>
        </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {loadingStats ? (
              ...
              <>
                <StatCard
                  icon="🏥"
                  value={displayStats.totalClinics}
                ...
                <StatCard
                  icon="⏳"
                  value={displayStats.currentQueue}
                  label="Navbatda"
                  color="amber"
                />
                <StatCard
                  icon="✅"
                  value={displayStats.todayServed}
                  label="Bugun qabul"
                  color="emerald"
                />
              </>
      ...
      <section className="py-16 bg-slate-50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6">
          <div className="flex items-end justify-between mb-8">
            ...
            <Link
              to="/clinics"
              className="hidden sm:flex items-center gap-1 text-brand-600 font-semibold hover:gap-2 transition-all text-sm">
            ...
            {[
              {
                ...
                title: "Klinikani tanlang",
                desc: "Xaritada yoki ro'yxatdan yaqin atrofingizdagi klinikani toping.",
              },
              {
                step: "02",
                icon: <Users className="w-7 h-7" />,
                title: "Shifokor tanlang",
                desc: "Mutaxassislik va navbat holatiga qarab qulay shifokorni tanlang.",
              },
              {
                step: "03",
                icon: <CheckCircle className="w-7 h-7" />,
                title: "Navbat oling",
                desc: "Faqat ismingizni kiriting va navbat raqamingizni oling.",
              },
            ].map((item, i) => (
              <motion.div
                key={i}
                ...
                <h3 className="font-display font-600 text-slate-900 text-lg mb-2">
                  {item.title}
                </h3>
            ...
            {[
              {
                ...
                title: "Registratsiyasiz",
                desc: "Hech qanday akkaunt kerak emas. Darhol ishlating.",
              },
              {
                icon: <TrendingUp className="w-6 h-6" />,
                title: "Real-time navbat",
                desc: "Navbat soni jonli ravishda yangilanib turadi.",
              },
              {
                icon: <MapPin className="w-6 h-6" />,
                title: "Interaktiv xarita",
                desc: "Toshkentdagi barcha klinikalar bir xaritada.",
              },
              {
                icon: <Shield className="w-6 h-6" />,
                title: "Ishonchli tizim",
                desc: "Klinika ma'lumotlari doim dolzarb.",
              },
            ].map((f, i) => (
              <motion.div
                key={i}
  ...
  return (
    <div className="flex items-center gap-3 bg-slate-50 rounded-2xl p-4">
      ...
      <div className="flex-1">
        ...
        <div className="font-display font-700 text-slate-900 text-lg leading-tight">
          {value}
        </div>
      </div>
...
function StatCard({
  ...
  return (
    <motion.div
      ...
      <div
        className={`w-11 h-11 rounded-2xl ${colors[color]} flex items-center justify-center text-xl mb-3`}>
        {icon}
      </div>
...
function SectionHeader({
  ...
  return (
    <div>
      <div
        className={`inline-block text-sm font-semibold px-3 py-1 rounded-full mb-3 ${light ? "bg-white/20 text-white/80" : "bg-brand-50 text-brand-600"}`}>
        {badge}
      </div>
      <h2
        className={`font-display text-2xl sm:text-3xl font-700 mb-2 ${light ? "text-white" : "text-slate-900"}`}>
        {title}
      </h2>
      <p className={`text-base ${light ? "text-brand-100" : "text-slate-500"}`}>
        {subtitle}
      </p>
    </div>
  );
